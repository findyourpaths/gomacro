...
typeutil contains patched version of some Go utilities to handle `github.com/findyourpaths/gomacro/go/types.Type`
...
2. Map: a mapping from `go/types.Type` to `interface{}` values,
   ...
   Since `github.com/findyourpaths/gomacro/go/types.Type` are not canonical,
   ...
   `github.com/findyourpaths/gomacro/go/typeutil/Map`, or something analogous
